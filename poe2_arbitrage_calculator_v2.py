...
def add_item():
    ...
    frame = tk.LabelFrame(items_container, text="Breach Splinters", padx=10, pady=5)
    frame.pack(fill="x", padx=5, pady=5)
    
    # Item type dropdown
    tk.Label(frame, text="Item Type:").grid(row=0, column=0, sticky="w")
    item_type = ttk.Combobox(frame, width=18, state="readonly")
    item_type['values'] = ("Breach Splinters", "Ritual Splinters", "Simulacrum Splinters", "Expedition Splinters")
    item_type.current(0)
    item_type.grid(row=0, column=1, pady=2)
    
    # Update frame title when dropdown changes
    ...
    items_per_chaos.grid(row=1, column=1, pady=2)
    ...
    current_ratio.grid(row=2, column=1, pady=2)
...
tk.Label(global_frame, text="1 Divine =").pack(pady=2)
